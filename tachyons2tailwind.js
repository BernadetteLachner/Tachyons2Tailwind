...
//all colors, border-colors & background colors: [/(?:\s|\"|\'|hover\-)(|bg-|b--)(\b((color))\b)(?:\s|"|\'|-l|-m|-sn)/g, "color"],
    ...
    const textarea = document.querySelector("#myDiv textarea");
...
//do stuff here for padmar and color, but for now, let's just grab the whole word match again, keep those hypens in place!
                //}
                    ...
                    if (match = qslicesp) {
                           // let text = textarea.value.split(match).join('"'+item[1]+' ');
                            let text = textarea.value.replace(match, ('"'+item[1]+' '));
                            textarea.value = text;
                    }
                    if (match = qsliceq) {
                            //let text = textarea.value.split(match).join('"'+item[1]+'"');
                    ...
                    if (match = hslice) {
                            //let text = textarea.value.split(match).join('hover\:'+item[1]);
                            let text = textarea.value.replace(match, ('hover\:'+item[1]));
                            textarea.value = text;
                    }
                    if (match = slicens) {
                            //let text = textarea.value.split(match).join('sm\:'+item[1]);
                            let text = textarea.value.replace(match, ('sm\:'+item[1]));
                            textarea.value = text;
                    }
                    if (match = slicem) {
                            //let text = textarea.value.split(match).join('md\:'+item[1]);
                            let text = textarea.value.replace(match, ('md\:'+item[1]));
                            textarea.value = text;
                    }
                    if (match = slicel) {
                            //let text = textarea.value.split(match).join('lg\:'+item[1]);
                            let text = textarea.value.replace(match, ('lg\:'+item[1]));
                            textarea.value = text;
                    }
